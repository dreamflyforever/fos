...
enum {
  ...
  MG_EV_POLL  = 1,        // mg_mgr_poll iteration        unsigned long *millis
  MG_EV_RESOLVE = 2,     // Host name is resolved        NULL
  MG_EV_CONNECT = 3,     // Connection established       NULL
  MG_EV_ACCEPT = 4,      // Connection accepted          NULL
  MG_EV_READ = 5,        // Data received from socket    struct mg_str *
  MG_EV_WRITE = 6,       // Data written to socket       int *num_bytes_written
  MG_EV_CLOSE = 7,       // Connection closed            NULL
  MG_EV_HTTP_MSG = 8,    // HTTP request/response        struct mg_http_message *
  MG_EV_HTTP_CHUNK = 9,  // HTTP chunk (partial msg)     struct mg_http_message *
  MG_EV_WS_OPEN = 10,     // Websocket handshake done     struct mg_http_message *
  MG_EV_WS_MSG = 11,      // Websocket msg, text or bin   struct mg_ws_message *
  MG_EV_WS_CTL = 12,      // Websocket control msg        struct mg_ws_message *
  MG_EV_MQTT_CMD = 13,    // MQTT low-level command       struct mg_mqtt_message *
  MG_EV_MQTT_MSG = 14,    // MQTT PUBLISH received        struct mg_mqtt_message *
  MG_EV_MQTT_OPEN = 15,   // MQTT CONNACK received        int *connack_status_code
  MG_EV_SNTP_TIME = 16,   // SNTP time received           struct timeval *
  MG_EV_USER = 17,        // Starting ID for user events
};
